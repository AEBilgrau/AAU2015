...
```{r, echo=-1}
par(mar=c(0,0,0,0)) # To remove some whitespace, this line is not printed
plot(X, chars = ".")
```
...
The `print.ppp` displays the number of recorded trees:

```{r}
print(X)
```

I.e. there are 14 trees in the dataset.
...
The average intensity can be computed via `intensity.ppp` v
...
There are subtle differences in the actual character/point types plotted. When given a string literal, the actual character is plotted as the spoint type.
...
# w <- square(30)  # Alternative, unitnames are not created
```
