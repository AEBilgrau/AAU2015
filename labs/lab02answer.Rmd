...
```{r, echo=-1}
par(mar=rep(0,4)) # remove some whitespace, this lines is not included 
plot(japanesepines)
```
...
```{r}
q <- quadratcount(japanesepines, nx = 3)
print(q)
```
...
As the arguments are passed to the generic `persp` function the `theta`,
`phi` and `shade`.
From the documentation they are "angles defining the viewing direction. `theta` gives the azimuthal direction and `phi` the colatitude."
